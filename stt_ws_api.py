...
global_time = (int(round(time.time() * 1000))) 
...
class StreamingServer(object):
    ...
    async def run(self, port: int):
        ...
        TRANSLATE_SERVER_URI = "ws://localhost:8766"
        try:
            ...
            print("Connection refused. Is the WebSocket server running at ws://localhost:8766?")
    ...
    async def handle_connection_impl(
        ...
        while True:
            ...
            # of the received audio samples equal to --sample-rate
                ...
                if (global_time - (int(round(time.time() * 1000)))) % 3 != 0:
                    continue
                request_time = time.time()
